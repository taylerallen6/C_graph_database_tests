...
Array_reverse_stack* array_reverse_stack_create(int total_length)
{
    ...
    new_stack->data = calloc(sizeof(int), new_stack->total_length);
...
{
    if (array_reverse_stack_is_empty(stack))
        return false;

    stack->current_length--;
    stack->head++;
    *output = stack->data[stack->head];
...
void array_reverse_stack_increase_total_length(Array_reverse_stack* stack, int increase_amount)
{
    ...
    stack->data = realloc(stack->data, sizeof(int) * stack->total_length);
...
//     // {
//     //     array_reverse_stack_dequeue(stack1, &result);
//     //     printf("%d\n", result);

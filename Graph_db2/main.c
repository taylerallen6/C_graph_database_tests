...
int main()
{
    ...
    Managed_index_array* nodes_arr = managed_index_array_create(num_slots);
    ...
    float connection_weights[num_slots];
    


    int new_index;
    managed_index_array_index_create(connections_arrs, &new_index);
    connection_weights[new_index] = 315.52;

    managed_index_array_index_create(connections_arrs, &new_index);
    connection_weights[new_index] = 9385.52;

    managed_index_array_index_create(connections_arrs, &new_index);
    connection_weights[new_index] = 33805.2;

    managed_index_array_index_create(connections_arrs, &new_index);
    connection_weights[new_index] = 564.52;

    managed_index_array_index_create(connections_arrs, &new_index);
    connection_weights[new_index] = 87363.2;


    for (int i = 0; i < connections_arrs->num_used_slots; i++)
    {
        if (managed_index_array_slot_is_available(connections_arrs, i) == false)
            printf("%f, ", connection_weights[i]);
    }
    printf("\n");
